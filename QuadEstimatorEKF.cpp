...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
	...
	Mat3x3F rotMat = Mat3x3F::Zeros();

	//First row
	rotMat(0, 0) = 1;
	rotMat(0, 1) = sin(phi)*tan(theta);
	rotMat(0, 2) = cos(phi)*tan(theta);

	//Second row
	//rotMat(1,0) already has zero
	rotMat(1, 1) = cos(phi);
	rotMat(1, 2) = -sin(phi);

	//Third row
	//rotMat(2, 0) already has zero
	rotMat(2, 1) = sin(phi) / cos(theta);
	rotMat(2, 2) = cos(phi) / cos(theta);

	V3F eulerDot = rotMat * gyro;

	float predictedRoll = rollEst + dtIMU * eulerDot.x;
	float predictedPitch = pitchEst + dtIMU * eulerDot.y;
	ekfState(6) = ekfState(6) + dtIMU * eulerDot.z;	
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  //Since curState has been assigned to predictedState;
  predictedState(0) += curState(3)*dt; //x = x + xDot*dt, similar for y and z
  predictedState(1) += curState(4)*dt;
  predictedState(2) += curState(5)*dt;

  V3F inertialAcc = attitude.Rotate_BtoI(accel);

  predictedState(3) += inertialAcc.x*dt; // x component of inertial acceleration * dt, similar for y and z.
  predictedState(4) += inertialAcc.y*dt;
  predictedState(5) += (inertialAcc.z-CONST_GRAVITY)*dt; //Effective acceleration taking gravity into account.
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  //Assign roll, pitch and yaw to common names
  float phi = roll;
  float theta = pitch;
  float psi = yaw;

  //Define the sine and cosine of angles to make easier to write and follow
  ...
  //Now fill in the RbgPrime matrix
  //First row
  ...
  RbgPrime(1, 1) = (sinPhi * sinTheta * cosPhi) - (cosPhi * sinPsi);
  RbgPrime(1, 2) = (cosPhi * sinTheta * cosPsi) + (sinPhi * sinPsi);

  //Third row
  //Third row is all 0 and already set because of  RbgPrime.setZero();
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  //From Eq. 51 of "Estimation for Quadrotors"
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;

  gPrime(3, 6) = (RbgPrime(0)*accel).sum() * dt; //Sum all of the row values together and multilply by dt
  gPrime(4, 6) = (RbgPrime(1)*accel).sum() * dt;
  gPrime(5, 6) = (RbgPrime(2)*accel).sum() * dt;

  //From predict part of section 3 of "Estimation for Quadrotors"
  ekfCov = gPrime * ekfCov * gPrime.transpose() + Q; //Q is measurement covariance. gPrime.transposeInPlace() generates an error.
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  for (int i = 0; i < 6; i++)
  {
	  //From equation 53-54 of "Estimation for Quadrotors"
	  zFromX(i) = ekfState(i);

	  //From equation 55 of "Estimation for Quadrotors", i=6 generates an error.
	  hPrime(i, i) = 1;
  }
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  hPrime(0, 6) = 1; //hPrime = [0 0 0 0 0 0 1]

  zFromX(0) = ekfState(6); //Estimated current yaw
